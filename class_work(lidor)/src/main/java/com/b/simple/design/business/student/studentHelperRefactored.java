...
public class StudentHelperRefactored {
  private static final int EXTRA_FOR_MARKS = 10;
  ...
  public boolean isGradeB(int mark, boolean isMaths) {
    ...
    return mark >= GRADE_B_LOWER_LIMIT && mark <= uppeLimit;
  }
  /* PROBLEM 2 */
	/*
  ...
  public String willQualifyForQuiz(int mark1, int mark2, boolean isMaths) {
    if(isNotGood(mark1, isMaths) || isNotGood(mark2, isMaths))
      return "NO";
    if(isGood(mark1, isMaths) || isGood(mark2, isMaths))
      return "YES";
    return "MAYBE";
  }
  private boolean isGood(int mark, boolean isMaths) {
    int extraLimit = isMaths ? 5 : 0;
    return mark >= 80 + extraLimit;
  }
  private boolean isNotGood(int mark, boolean isMaths) {
    int extraLimit = isMaths ? 5 : 0;
